...
export const LaporanPembelian = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const validItems = formData.barang.filter((item) => item._id && item.vol > 0);
    if (validItems.length === 0) {
      setErrorMessage("Harap masukkan item yang valid.");
      setFormLoading(false);
      return;
    }
  ...
  if (isError || katalogError)
    return (
      <p className="text-red-500">
        {error?.message || katalogError?.message}
      </p>
    );
  ...
  return (
    <div className="container mx-auto my-4 p-4">
      ...
      {openModal && (
        ...
        >
          <div
            className="bg-white p-6 rounded-lg w-full max-w-md sm:max-w-lg md:max-w-xl max-h-[80vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-2xl font-semibold mb-4">Tambah Laporan Pembelian</h2>
                ...
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                        ongkir: e.target.value === "" ? 0 : parseFloat(e.target.value),
                      }))
                    }
                    className="border border-gray-300 p-2 rounded-md"
                    placeholder="Ongkir"
                ...
                <div className="space-y-4">
                  ...
                  {formData.barang.map((item, index) => (
                    <div className="flex flex-wrap gap-2 items-center" key={index}>
                      <select
                        className="flex-1 min-w-[150px] border border-gray-300 p-2 rounded-md"
                        value={item._id}
                      ...
                      <input
                        type="number"
                        className="w-[80px] border border-gray-300 p-2 rounded-md"
                        value={item.vol}
                        onChange={(e) =>
