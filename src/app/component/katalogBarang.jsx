...
export const KatalogBarangList = () => {
  ...
  const [newSisaQty, setNewSisaQty] = useState("");
  const [newTglBeli, setNewTglBeli] = useState("");
  ...
  const handleOpenModal = (barang) => {
    ...
    setHargaList(
      Array.isArray(barang?.harga)
        ? barang.harga.map((h) => ({
            tgl_beli: h.tgl_beli ? new Date(h.tgl_beli).toISOString().split("T")[0] : "",
            harga: h.harga || "",
            sisa_qty: h.sisa_qty || "",
          }))
        : []
  ...
  const handleAddHarga = () => {
    if (newHarga && newSisaQty && newTglBeli) {
      setHargaList([
        ...hargaList,
        {
          tgl_beli: newTglBeli,
          harga: Number(newHarga),
          sisa_qty: Number(newSisaQty),
        },
      ]);
      setNewHarga("");
      setNewSisaQty("");
  ...
  const handleRemoveHarga = (index) => {
    setHargaList(hargaList.filter((_, i) => i !== index));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const barangData = {
      ...
      satuan,
      harga: hargaList.map((h) => ({
        tgl_beli: h.tgl_beli ? new Date(h.tgl_beli) : new Date(),
        harga: Number(h.harga),
        sisa_qty: Number(h.sisa_qty),
      })),
            ...
            {filteredData?.map((item, index) => (
              <tr key={item._id} className="odd:bg-white even:bg-gray-50">
                ...
                <td className="border px-3 py-2">
                  {Array.isArray(item.harga) && item.harga.length > 0 ? (
                    <ul className="list-disc pl-4">
                      {item.harga.map((h, i) => (
                        <li key={i}>
                          {Number.isFinite(h.harga)
                            ? `Rp ${h.harga.toLocaleString("id-ID")}`
                            : "Harga tidak valid"} 
                          (Sisa: {Number.isFinite(h.sisa_qty) ? h.sisa_qty : "N/A"}, 
                          Tanggal: {h.tgl_beli ? new Date(h.tgl_beli).toLocaleDateString("id-ID") : "N/A"})
                        </li>
                      ))}
                    </ul>
                  ) : (
                ...
                <td className="border px-3 py-2">{item.stok_awal || 0}</td>
                <td className="border px-3 py-2">{item.masuk || 0}</td>
                <td className="border px-3 py-2">{item.keluar || 0}</td>
                <td className="border px-3 py-2">
                  {(item.stok_awal || 0) + (item.masuk || 0) - (item.keluar || 0)}
                </td>
