...
import { TextField, Button, Grid, Typography, IconButton, Select, MenuItem   } from "@mui/material";
...
export const LaporanPenjualanForm = () => {
	...
	const handleSubmit = async (event) => {
		...
		try {
			const finalData = {
				...
				item: formData?.item?.map((item, index) => ({
					_id: item[index]._id,
					qty: item?.qty
				})),
				ppn: formData?.ppn,
	...
	console.log(formData?.item[0]._id);
						...
						<Grid container spacing={2} key={index}>
							<Grid item xs={4}>
								<Select
									...
									displayEmpty
									inputProps={{ "aria-label": "ID Barang" }}
								>
