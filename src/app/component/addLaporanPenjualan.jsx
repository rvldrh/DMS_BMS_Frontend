...
export const LaporanPenjualanForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    // Pastikan tidak ada item yang kosong
    const validItems = formData.item.filter((item) => item._id && item.qty > 0);

    if (validItems.length === 0) {
        alert("Harap tambahkan setidaknya satu item dengan ID dan jumlah yang valid.");
        return;
    }
    ...
    try {
        const finalData = {
            ...
            item: validItems.map((item) => ({
                _id: item._id, // Sekarang dijamin tidak undefined
                qty: item.qty,
            })),
